...
SAVE_FOLDER = EXTRACTED_SAVING_PATH.split("\\")[
    len(EXTRACTED_SAVING_PATH.split("\\")) - 2
]
...
def tensorflowFormat(img_dict):
    ...
    with open("train.csv",mode='w+',encoding='utf8') as f:
        ...
        for img_id in img_dict:
            ...
            f.write(f"{image_meta['file_name']},{image_meta['height']},{image_meta['width']},")
    ...
    for img_id in img_dict:
        ...
        for i in range(len(annotations)):
            ...
            if entity in target_classes:
                ...
                node_xmax.text = str(
                    round(annotations[i]["bbox"][0] + annotations[i]["bbox"][2])
                )
                node_ymax = SubElement(node_bndbox, "ymax")
                node_ymax.text = str(
                    round(annotations[i]["bbox"][1] + annotations[i]["bbox"][3])
                )

        xml = tostring(node_root, pretty_print=True)
        dom = parseString(xml)
        with open(
            EXTRACTED_SAVING_PATH + image_meta["file_name"].split(".")[0] + ".xml", "w"
        ) as xml_file:
            xml_file.write(dom.toxml())

        copyfile(
            COCO_IMAGES_DIRECTORY + image_meta["file_name"],
